...
gsgd_rmdptre      <- taxaInfo( "./clade/pH5_7326_gsgd_rmdP_e0627.tre", useTree = TRUE )
gsgd_rmdptre[[7]][ is.na( gsgd_rmdptre[[7]] ) ] <- "others"
gsgd_rmdptre[[7]] <- c( "others", "234", "232")[ match( gsgd_rmdptre[[7]], unique( gsgd_rmdptre[[7]] ) ) ]
...
tbb$anno <- factor( tbb$anno, levels = c("others", "232", "234") )
...
tbb %>% 
  ...
  scale_fill_manual( values = c( pyCol( c( "gray", "green", "red") ) ) ) + 
  scale_color_manual( values = c( pyCol( c( "gray", "green", "red") ) ) ) + 
  theme_minimal() +
...
tbp      <- tb
tbp$anno <- ifelse( tbp$anno == "232" | tbp$anno == "234", "23", "others")
...
o.CN    <- setdiff( geo_CN, c( c234, c232 ) )
...
trefile$branch.size[ c(c234, c232) ] <- 0.6
...
ggtree( gsgd_rawtr ) %<+% trefile + aes( color = branch.col, size = I(branch.size) ) + 
  ...
  geom_tippoint( shape = 21) + 
  scale_color_manual( values = c("#9be39b", "#eb9394", "#1c641c", "#6c1414", "#404040", "#bfbfbf" ) )
  


# combine
ggarrange( s3,  
           ggarrange( s1, s2, ncol = 1, nrow = 2, heights = c(2,1) ),
           ncol = 2, widths = c(3,5) ) #a5 landscape
